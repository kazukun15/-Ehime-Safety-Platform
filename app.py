# -*- coding: utf-8 -*-
"""
愛媛セーフティ・プラットフォーム (ESP) - Map Fix Edition
Version: 13.0
Author: World Class Program Designer
Description: 地図表示バグ修正版（Googleマップ/OSM対応）、APIキー不要化
"""
...
class AppConfig:
    ...
    SUBTITLE = "Map Fixed v13.0"
    USER_AGENT = "ESP/13.0-MapFix"
    TIMEOUT = 10
    MAX_WORKERS = 4
...
def inject_css():
    ...
    <style>
      ...
      .stTabs [data-baseweb="tab"] {
        height: 48px; flex: 1; background-color: #eaeff3; border-radius: 8px; 
      ...
      /* ティッカー */
      .ticker-wrap {
        width: 100%; overflow: hidden; background: var(--card); border-y: 1px solid var(--border);
...
def build_snap_lines(jpoints: List[Dict], ways: List[Dict]) -> List[Dict]:
    ...
    for jp in jpoints:
        ...
        best_dist = 300; best_proj = None; best_way_vec = None
        for w in ways:
            coords = w["coords"]
        ...
        if best_proj and best_way_vec:
            ...
            if vec_len > 0:
                ...
                lines.append({
                    ...
                    "color": [255, 50, 50, 200], "width": 5 + min(15, total // 50)
                })
    return lines

@dataclass
class Incident:
    category: str; summary: str; municipality: str; lon: float; lat: float; style: Dict; src: str
...
def main():
    ...
    if show_jartic: ticker_text += "<span class='ticker-item' style='color:var(--accent); font-weight:bold;'>【交通】JARTICリアルタイム情報 連携中</span>"
    ...
    with tab_map:
        ...
        # 1. TileLayer (確実に地図を表示するための最重要レイヤー)
        tile_cfg = AppConfig.TILESETS[map_style_name]
        layers.append(pdk.Layer(
            "TileLayer",
            data=tile_cfg["url"],
            min_zoom=0, max_zoom=tile_cfg["max_zoom"],
            opacity=1.0,
            # 画像読み込み時のちらつきを抑える設定
            get_line_color=[0, 0, 0], filled=True
        ))
        ...
        # ★APIキーなしで表示させるための重要設定: map_provider=None, map_style=None
